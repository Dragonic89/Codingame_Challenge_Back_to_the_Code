...
class Player
{
    ...
    struct CaseDatas
    {
        ...
        public bool tagRoomCentre;//contient l'info si les 8 voisins sont vides (justifie la présence d'une room)
        public int tagRoomStart;//contient l'info pour savoir si on doit éviter ce point d'entrée (si >= 3 : correspond aux voisins, parmis les 8, déjà en notre possession)
    ...
    enum MODE
    {
        ...
        next,//objectif à atteindre déjà renseigné
    }
    static void Main(string[] args)
    {
        ...
        if(V6)//nouvel algo, recherche des zones d'intérêt et contournement de ces zones
        {
            int width = 35;
            int height = 20;
            int lastGameRound = 0;
            ...
            //on rajoute à l'historique son état initial (gameRound 0)
            ...
            while(true)
            {
                ...
                //correction du gameRound à cause du referee qui le fait sauter au max d'un coup si le jeu est fini avant le nombre max de rounds
                ...
                for (int i = 0; i < opponentCount; i++)
                {
                    ...
                    //backInTime utilisé par un adversaire
                    if(!backInTimeUsed[i+1] && opponentBackInTimeLeft==0)
                    {
                        //backInTime appliqué (peut être un tour après la détection)
                        if(gameRound!=lastGameRound+1)
                        {
                            Console.Error.WriteLine("BackInTime détecté");
                    
                            backInTimeUsed[i+1] = true;
                            
                            //mise à jour des infos par rapport à ce retour en arrière
                ...
                //vérification si positionnement de départ symétrique
                if(isSymetric==-1)
                {
                    //on considère symétrie si ennemi à la position inverse de nous
                    for (int i = 1; i < opponentCount; i++)
                    {
                ...
                //tableau des infos de chaque case (intérêt, appartenance, ...)
                ...
                for(int j=0;j<height;j++)
                {
                    for(int i=0;i<width;i++)
                    {
                        ...
                        //par défaut on sait déjà que les cases sur les côtés ne pourront pas avoir leurs 8 voisins vides
                ...
                for (int j = 0; j < height; j++)
                {
                    ...
                    for(int i=0; i< width; i++)
                    {
                        if(line[i]=='.')//vide
                        {
                            //casesData[i,j] = -1;//déjà renseigné par défaut en vide
                ...
                //on entoure les case des adversairs libre d'un tag pour pas les parcourir par sécurité en mode Room
                ...
                if(mode==MODE.room)// on va s'obliger à fermer virtuellement certaines cases pour pas faire de zones trop grandes
                {
                    ...
                    //TODO il faut améliorer cette partie
                    
                    //réglage de la taille des zones à fermer
                    if(opponentCount==1)
                        offset = ((gameRound>nbrManchesMax/3)?8:10);//paramétrage à régler
                    else if(opponentCount==2)
                        offset = ((gameRound>nbrManchesMax/3)?8:9);//paramétrage à régler
                    else 
                        offset = ((gameRound>nbrManchesMax/3)?8:8);//paramétrage à régler
                            ...
                            for(int jj=j-1;jj<=j+1;jj++)
                            {
                                for(int ii=i-1;ii<=i+1;ii++)
                                {
                                    if(0<=ii && ii<width && 0<=jj && jj<height)
                                    {
                                        ...
                                        {
                                            //gestion de tag roomCentre
                                            casesDatas[ii,jj].tagRoomCentre=false;//les voisins d'une case prise ne peuvent être un centre de room
                                        }
                ...
                //ici les centres de room sont bien renseignés
                    ...
                    for(int i=0; i< width; i++)
                    {
                        if(casesDatas[i,j].tagRoomCentre)
                        {
                            ...
                            if(casesDatas[i,j-1].tagRoomCentre)
                            {
                                ...
                                //on doit checker le voisin de gauche voir si il a aussi une origine peut être différente
                                if(casesDatas[i-1,j].tagRoomCentre && casesDatas[i-1,j].roomOrigin!=casesDatas[i,j].roomOrigin)
                                {
            	                    //dans ce cas on va convertir les cases de la room de gauche à celle du haut
                            ...
                            else//on crée une nouvelle room
                            {
                ...
                //on mettra de côté les cases de bord de room pour faciliter la recherche du plus proche plus tard
                    ...
                    for(int i=0; i< width; i++)
                    {
                        if(casesDatas[i,j].tagRoomCentre)
                        {
                            ...
                            for(int jj=j-1;jj<=j+1;jj++)
                            {
                                for(int ii=i-1;ii<=i+1;ii++)
                                {
                                    if(!casesDatas[ii,jj].tagRoomCentre && ((ii==x && jj==y) || casesDatas[ii,jj].appartenance==-1))//bord trouvé
                                    {
                                        ...
                                        //si pas déjà rajouté dans la liste des bords de room
                                        //et que son tagStart est inférieur à trois
                ...
                //a ce niveau roomEdges contient les possibles points d'entrées de toutes rooms confondues
                //et roomsEdges contient les bords pour chaque room (pas super bien choisis les noms ici, m'enfin pas grave à ce niveau)
                
               
                //verification si case atteinte prise
                if(objectifCase.x!=-1 && casesDatas[objectifCase.x,objectifCase.y].appartenance!=-1)
                {
                    //TODO vérif si objectif atteint de type roomEdge
                ...
                if(mode==MODE.room && gameRound>1)
                {
                    ...
                    //si on est entouré de cases non vides, on a soit fini, soit on a été bloqué d'une façon ou d'une autre
                ...
                //si room non finie à priori
                if(mode==MODE.room)
                {
                    //on contourne les cases jusqu'à espérer pouvoir fermer la room
                ...
                if(mode==MODE.search)
                {
                    //on recherche la room la plus proche à une distance et taille respectable
                    ...
                    if(roomEdges.Count>0)
                    {
                        ...
                        foreach(Case c in roomEdges)
                        {
                            if(c.x!=x || c.y!=y)
                            {
                                ...
                                {
                                    //bonus de recherche si room déjà entamée et gain superieur à 3 attendu
                                    ...
                                    if(a < minDistance)//paramètre à adapter en fonction des tests
                                    {
                                        minDistance = a;
                        ...
                        {
                            //pour cette room à distance min, on va vérifier si on a pas déjà des cases à nous pour changer l'objectif
                            ...
                            if(minDistance2 != 10000 && minDistance2-minDistance < 8)//paramètre à changer en fonction des tests
                            {
                ...
                //cas particulier, si on est en conflit avec un ennemi, on revient en arrière
                if(gameRound>1 && casesDatas[x,y].appartenance==-1)
                {
                    Console.Error.WriteLine("Conflit détecté");
                ...
                if(objectifCase.x==-1)//si aucun objectif tourvé, mode random
                {
        ...
        else//ancien algo, pas mal de bordel ^^' ! Fais des rectangles et attaque en 1v1
        {
            ...
            bool roomMode = true;//mode qui permet de dire à notre IA de s'occuper de finaliser une room
            int sensTourne = 0;//0-> indéfini, 1-> horaire, 2-> antihoraire
            ...
            while (true)
            {
                ...
                //correction du gameRound à caue du referee qui le fait sauter au max d'un coup si le jeu est fini avant le nombre max de rounds
                gameRound = Math.Min(lastGameRound+1,gameRound);
                ...
                //initialisation de l'info sur la position de départ
                ...
                for (int i = 0; i < opponentCount; i++)
                {
                    ...
                    //gestion backInTime ennemi
                    if(!backInTimeUsed[i+1] && opponentBackInTimeLeft==0)//backInTime utilisé par un adversaire
                    {
                        if(lastGameRound!=gameRound-1)//backInTime appliqué
                        {
                            Console.Error.WriteLine("BackInTime détecté");
                            ...
                            //mise à jour du cmportement de notre IA par rapport à l'historique
                            //on prendr à 2 round de différence à cause du décalage
                            
                            //cas particulier, si c'est le round 1, on revient à zéro
                            if(gameRound==1)
                            {
                                //cas particulier 1v1
                                if(opponentCount==1)
                ...
                {
                    roomMode = true;///activation du roomMode pour vérification
                    objectifCase.x=-1;
                    objectifCase.y=-1;
                }
                ...
                if(roomMode)
                {
                    ...
                    if(roomFinish)
                    {
                        Console.Error.WriteLine("Room terminée");
                ...
                if(roomMode)//recherche de la prochaine position
                {
                    ...
                    //vérification si l'on passe depuis 3 tours à côté d'un voisin alors qu'on tente de tourner vers lui
                    if(sensTourne==1)
                    {
                    ...
                    //vérification si l'on tente de passer proche de nos cases déjà prises
                    ...
                    //traitement spécial première room
                    if(firstRoom)
                    {
                        if(gameRound==1)//initialisation, avec inversion  si back in time utilisé par adversaire
                        {
                            if(firstPosition.x<=17 && firstPosition.y<10)//on débute en haut à gauche
                            {
                            ...
                            else if(firstPosition.x<=17 && firstPosition.y>=10)//on débute en bas à gauche
                            {
                            ...
                            else if(firstPosition.x>17 && firstPosition.y>=10)//on débute en bas à droite
                            {
                            ...
                            else//on débute en haut à droite
                            {
                    ...
                    else// en dehors de la première room
                    {
                        if(currentDirection==0)//arrivée dans la room à fermer
                        {
                            //lire historique dernière position pour voir d'où on vient
                    ...
                    //cas particulier, si je n'ai pas réussi à activer la case sur laquelle je suis, donc un autre joueur avec moi, je reviens en arrière
                    ...
                    else
                    {
                        ...
                        //force changement de direction après un certains moments
                        ...
                        {
                            if(backInTimeLeft==0)//si back utilisé, on réduit la firstRoom
                            {
                                if(firstPosition.x<=17 && firstPosition.y<10)//on débute en haut à gauche
                                {
                        ...
                        //tant qu'on a pas trouvé un objectif, on vérifie si l'on doit tourner (limité aux 3 changements de direction possibles)
                        while(objectifCase.x==-1 && nbrChangeDir<4)
                        {
                            //vérification si prochaine case à plus d'un voisin
                            ...
                            //vérification si autorisation de continuer dans la direction actuelle
                        ...
                        //si on a pas trouvé d'objectif, ce'st forcément que la room nous a été bloquée d'une façon qu'on a pas su gérer ici
                        if(objectifCase.x==-1)
                        {
                            //on désactive le mode room pour repasser en mode recherche
                            roomMode = false;
                            firstRoom = false;
                            currentDirection = 0;
                            timeInRoom = 0;
                ...
                if(objectifCase.x!=-1 && gameRound>7)//ne s'applique que si on a un objectif en cours
                {
                    ...
                    if(nbrOccurences>=2)
                    {
                        Console.Error.WriteLine("blocage détecté");
                        objectifCase.x = -1;
                        objectifCase.y = -1;
                        roomMode = false;
                        for(int i=2;i<=7;i++)
                ...
                if(oneVSone)
                {
                    ...
                    {
                        //il ne faut surtout pas essayer de s'en rapprocher en le ciblant directement car le déplacement sera horizontal avant d'être vertical, ce qui nous arrange pas
                ...
                //vérification si on doit rechercher une case vide
                if( !oneVSone &&
                ((!roomMode && objectifCase.x!=-1 && statutCase[objectifCase.x][objectifCase.y]!=-1)//si la case qu'on cherchait à été remplie
                || (!roomMode && nbrVides>=1 && objectifCase.x==-1)))//si pas d'objectif en cours
                {
                    ...
                    //préparation calcul de distances
                    ...
                    while(nbrVides>=1 && objectifCase.x==-1)
                    {
                        ...
                        //mode de recherche préférant N,E,S,W en focntion des situations
                        int xx=0;
                        int yy=0;
                        if(y<10)
                        {
                        ...
                        //enregistrement de la première case trouvée
                        if(minCaseSave.x==-1)
                        {
                        ...
                        //vérification par rapport aux adversaires
                        for (int i = 1; i <= opponentCount; i++)
                        {
                            ...
                            {
                                statutCase[minCase[0].x][minCase[0].y]=-2;//on l'élimine du process
                                nbrVides--;
                                break;
                            }
                        }
                        
                        //on a trouvé la case min qu'on voulait
                    ...
                    //si on a pas trouvé d'objectif min pratique
                    if(objectifCase.x==-1)
                    {
                        //on prend le plus proche quand même si il existe
                        if(minCaseSave.x!=-1)
                        {
                ...
                if(objectifCase.x==-1)//si on a pas trouvé d'objectif,  au hasard
                {
                    Console.Error.WriteLine("Random");
                    ...
                    objectifCase.x = -1;//important, on doit tout de suite remettre l'objectif à -1 pour forcer la prochaine recherche
                    objectifCase.y = -1;
                }
                else
                {
